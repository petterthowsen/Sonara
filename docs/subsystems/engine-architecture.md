...
The audio callback (`Engine/src/audio/processing.rs`, set up by `engine.rs`) processes audio in this order:
...
- At the start of each callback `processing.rs::schedule_live_midi_events` drains every queue into `Channel::scheduled_midi_events`. Live input plays with a fixed latency of one buffer: an event that arrived `dt` before the callback lands `dt` before the end of the buffer, and events older than one buffer play at frame 0. Queue order is arrival order, so no sort is needed.
...
**`mixing.rs::mix_and_output` orchestrates four passes, keeping the audio thread real-time safe:**
