...
impl PluginProcess {
    ...
    pub fn send_command(&mut self, cmd: PluginCommand) -> Result<(), String> {
        ...
        info!("[ProcessManager] Sending to {} (socket fd={}): {:?}", self.plugin_id, fd, cmd);
    ...
    pub fn recv_response(&mut self) -> Result<PluginResponse, String> {
        ...
        // Read byte-by-byte until newline (no BufReader to avoid buffering issues)
        loop {
...
impl ProcessManager {
    ...
    pub fn spawn_plugin(
        ...
        let pid = child.id();
        info!("Plugin subprocess spawned: PID={} with Unix socket FD={}", pid, unix_sock_child_fd);
        ...
        thread::spawn(move || {
            loop {
                ...
                let process_arcs: Vec<(String, Arc<Mutex<PluginProcess>>)> = {
                    ...
                    processes.iter().map(|(k, v)| (k.clone(), Arc::clone(v))).collect()
                };
