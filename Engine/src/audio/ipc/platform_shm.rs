...
impl PlatformSharedMemory {
    ...
    pub fn new(name: &str, size: usize) -> Result<Self, String> {
        ...
        info!(
            "🔍 Created OwnedFd for FD {} from memfd_create",
            fd.as_raw_fd()
        );
    ...
    pub fn from_fd(fd: RawFd, size: usize) -> Result<Self, String> {
        ...
        info!(
            "✅ Shared memory mapped at {:?}, OwnedFd.as_raw_fd()={}",
            ptr, fd_after_mmap
        );
...
impl Drop for PlatformSharedMemory {
    fn drop(&mut self) {
        ...
        info!(
            "✅ About to drop OwnedFd for FD {} (this will close it)",
            fd_num
        );
