...
[FunDSP](https://github.com/SamiPerttu/fundsp) is an audio DSP ([digital signal processing](https://en.wikipedia.org/wiki/Digital_signal_processing)) library for audio processing and synthesis.

FunDSP features a powerful inline graph notation for describing audio processing networks. The notation taps into composable, zero-cost abstractions that express the structure of audio networks as [Rust](https://www.rust-lang.org/) types.

Another innovative feature of FunDSP is its signal flow system, which can determine analytic [frequency responses](https://en.wikipedia.org/wiki/Frequency_response) for any [linear network](https://en.wikipedia.org/wiki/Linear_filter).
...
### [](https://crates.io/crates/fundsp#uses)Uses
...
### [](https://crates.io/crates/fundsp#rust-audio-discord)Rust Audio Discord

To discuss FunDSP and other topics, come hang out with us at the [Rust Audio Discord](https://discord.gg/3hfDXS6hhQ).

### [](https://crates.io/crates/fundsp#related-projects)Related Projects

[bevy\_fundsp](https://github.com/harudagondi/bevy_fundsp) integrates FunDSP into the [Bevy](https://bevyengine.org/) game engine.

[midi\_fundsp](https://github.com/gjf2a/midi_fundsp) enables the easy creation of live synthesizer software using FunDSP for synthesis.

[quartz](https://github.com/tomara-x/quartz) is a visual programming and DSP playground with releases for Linux, Mac and Windows.

## [](https://crates.io/crates/fundsp#installation)Installation
...
The `files` feature is enabled by default. It adds support for loading of audio files into `Wave` objects via the [Symphonia](https://crates.io/crates/symphonia) crate.

### [](https://crates.io/crates/fundsp#no_std-support)no\_std Support
...
## [](https://crates.io/crates/fundsp#graph-notation)Graph Notation

_FunDSP Composable Graph Notation_ expresses audio networks in algebraic form, using graph operators. It was developed together with the functional environment to minimize the number of typed characters needed to accomplish common audio tasks.

Many common algorithms can be expressed in a natural form conducive to understanding. For example, an [FM oscillator](https://ccrma.stanford.edu/~jos/sasp/Frequency_Modulation_FM_Synthesis.html) can be written simply (for some `f` and `m`) as:

    sine_hz(f) * f * m + f >> sine()
    

The above expression defines an audio graph that is compiled into a stack allocated, inlined form using the powerful generic abstractions built into [Rust](https://www.rust-lang.org/). Connectivity errors are detected during compilation, saving development time.

### [](https://crates.io/crates/fundsp#audio-dsp-becomes-a-first-class-citizen)Audio DSP Becomes a First-Class Citizen

With no macros needed, the FunDSP graph notation integrates audio DSP tightly into the [Rust programming language](https://www.rust-lang.org/) as a first-class citizen. Native Rust operator precedences work in harmony with the notation, minimizing the number of parentheses needed.
...
## [](https://crates.io/crates/fundsp#basics)Basics

### [](https://crates.io/crates/fundsp#component-systems)Component Systems
...
input and output arity fixed at compile time
...
input and output arity fixed after construction

* * *

All `AudioNode` and `AudioUnit` components use 32-bit floating point samples (`f32`).

The main property of a component in either system is that it is a processing node in a graph with a specific number of input and output connections, called its _arity_. Audio and control signals flow through input and output connections.
...
`AudioNode`s can be stack allocated for the most part. Some nodes may use the heap for audio buffers and the like.
...
### [](https://crates.io/crates/fundsp#conversions)Conversions

`AudioNode`s are converted to the `AudioUnit` system using the wrapper type `An`, which implements `AudioUnit`. Opcodes in the preludes return nodes already wrapped.

`AudioUnit`s can in turn be converted to `AudioNode` with the wrapper `unit`. In this case, the input and output arities must be provided as type-level constants `U0`, `U1`, ..., for example:
...
### [](https://crates.io/crates/fundsp#processing)Processing
...
If maximum speed is important, then it is a good idea to use block processing, as it reduces function calling, processing setup and dynamic network overhead, and enables explicit [SIMD](https://en.wikipedia.org/wiki/Single_instruction,_multiple_data) support.
...
### [](https://crates.io/crates/fundsp#block-processing)Block Processing
...
Buffers are always 64 samples long (`MAX_BUFFER_SIZE`), have an arbitrary number of channels, and are explicitly [SIMD](https://en.wikipedia.org/wiki/Single_instruction,_multiple_data) accelerated with the type `f32x8` from the [`wide`](https://crates.io/crates/wide) crate. The samples are laid out noninterleaved in a flat array.
...
To call `process` automatically behind the scenes, use the `BlockRateAdapter` adapter component. However, it works only with _generators_, which are components with no inputs.

To access `f32` values in a buffer, use methods with the `f32` suffix, for example, `at_f32` or `channel_f32`.

### [](https://crates.io/crates/fundsp#sample-rate-independence)Sample Rate Independence
...
## [](https://crates.io/crates/fundsp#audio-processing-environment)Audio Processing Environment
...
### [](https://crates.io/crates/fundsp#deterministic-pseudorandom-phase)Deterministic Pseudorandom Phase
...
## [](https://crates.io/crates/fundsp#operators)Operators
...
### [](https://crates.io/crates/fundsp#operators-diagram)Operators Diagram

![](https://github.com/SamiPerttu/fundsp/raw/HEAD/operators.png "FunDSP Graph Operators")
...
### [](https://crates.io/crates/fundsp#broadcasting)Broadcasting
...
### [](https://crates.io/crates/fundsp#thru)Thru
...
### [](https://crates.io/crates/fundsp#generators-filters-and-sinks)Generators, Filters and Sinks

Components can be broadly classified into generators, filters and sinks. _Generators_ have only outputs, while _filters_ have both inputs and outputs.
...
### [](https://crates.io/crates/fundsp#graph-combinators)Graph Combinators

Of special interest among operators are the four custom combinators: _pipe_ ( `>>` ), _bus_ ( `&` ), _branch_ ( `^` ), and _stack_ ( `|` ).

The pipe is a serial operator where components appear in _processing_ order. Branch, stack, and arithmetic operators are parallel operators where components appear in _channel_ order.
...
#### [](https://crates.io/crates/fundsp#pipe)Pipe
...
#### [](https://crates.io/crates/fundsp#branch)Branch
...
Branching is useful for building _banks_ of components such as filters.

The branch operation is also available as a function: `branch(A, B)` is equal to `A ^ B`.

#### [](https://crates.io/crates/fundsp#bus)Bus

The bus ( `&` ) operator can be thought of as an inline [audio bus](https://en.wikipedia.org/wiki/Audio_bus) with a fixed set of input and output channels. It builds signal buses from components with identical connectivity.
...
#### [](https://crates.io/crates/fundsp#stack)Stack

The stack ( `|` ) operator builds composite components. It can be applied to any two components.

As a graph operator, the stack corresponds to the _disjoint union_. In `A | B`, the inputs and outputs of `A` and `B` are disjoint and they are processed independently, in parallel.
...
## [](https://crates.io/crates/fundsp#expressions-are-graphs)Expressions Are Graphs
...
Connectivity is checked during compilation. Mismatched connectivity will result in a compilation error complaining about mismatched [`typenum`](https://crates.io/crates/typenum) [types](https://docs.rs/typenum/1.12.0/typenum/uint/struct.UInt.html). The arrays `Frame<f32, Size>` that connect components in `tick` come from the [`generic-array`](https://crates.io/crates/generic-array) and [`numeric-array`](https://crates.io/crates/numeric-array) crates.

### [](https://crates.io/crates/fundsp#computational-structure)Computational Structure
...
### [](https://crates.io/crates/fundsp#net)Net
...
The `Net` component offers an explicit graph interface for connecting `AudioUnit` nodes. Arity of the net is specified as constructor arguments.
...
The overhead of `Net` is the overhead of calling into `Box<dyn AudioUnit>` objects. Beyond that, the dynamic versions are roughly as efficient as the static ones.

The graph syntax is also available for combining `Net` instances. Connectivity checks are then deferred to runtime.

Networks can also be combined inline with components from the preludes. The components are first converted to `Net`.

When we need dynamic processing, we can start it with `Net::wrap`, which converts any unit into a network. Also, we can control when to cross from the static realm into the dynamic by the placement of `Net::wrap`.
...
For real-time situations, a `Net` can be divided into a frontend and a backend. The frontend handles changes to the network, while the real-time safe backend renders audio.
...
### [](https://crates.io/crates/fundsp#sequencer)Sequencer

The `Sequencer` component mixes together generator nodes dynamically. It can start and stop rendering nodes with sample accuracy.
...
Adding new events, their start and stop times as well as fade-in and fade-out envelopes can be set. The sequencer returns an `ID` that can be used for later edits to the events.
...
## [](https://crates.io/crates/fundsp#input-modalities-and-ranges)Input Modalities And Ranges

Some signals found flowing in audio networks.

Modality

Preferred Units/Range
...
The wavetable oscillator uses this range.
...
Inner processing may use any range that is convenient. However, only special output formats can store audio data outside this range.

stereo pan

\-1...1 (left to right)

For ergonomy, consider clamping any pan input to this range.

control amount

0...1

If there is no natural interpretation of the parameter.

## [](https://crates.io/crates/fundsp#working-with-waves)Working With Waves

FunDSP includes a multichannel wave abstraction called `Wave`. For example, to render 10 seconds of pink noise:
...
Saving of waves is possible in 16-bit or 32-bit [WAV](https://en.wikipedia.org/wiki/WAV) when the `std` feature is enabled. The 16-bit format is integer based while the 32-bit format is floating point.
...
Loading of audio files in various formats is handled by the [Symphonia](https://crates.io/crates/symphonia) crate. Symphonia integration is enabled by the `files` feature, which is enabled by default. The `std` feature must be enabled also.
...
## [](https://crates.io/crates/fundsp#signal-flow-analysis)Signal Flow Analysis
...
The system can calculate the [frequency response](https://en.wikipedia.org/wiki/Frequency_response) of any _[linear network](https://en.wikipedia.org/wiki/Linear_filter)_ analytically by composing [transfer functions](https://en.wikipedia.org/wiki/Transfer_function#Linear_time-invariant_systems) and folding constants. Linear networks are constructed from linear filters, delays, and the operations of:
...
For example, [FIR](https://en.wikipedia.org/wiki/Finite_impulse_response) filters can be composed inline from single sample delays (the `tick` opcode) and arithmetic. Signal flow analysis will readily reveal that a 2-point averaging filter has zero gain at the [Nyquist](https://en.wikipedia.org/wiki/Nyquist_frequency) frequency, while a 3-point averaging filter does not:
...
## [](https://crates.io/crates/fundsp#filters)Filters

### [](https://crates.io/crates/fundsp#list-of-linear-filters)List of Linear Filters
...
[Simper SVF](https://cytomic.com/files/dsp/SvfLinearTrapOptimised2.pdf)
...
Adjustable delay at DC in samples.
...
[biquad](https://en.wikipedia.org/wiki/Digital_biquad_filter)

Arbitrary biquad with fixed parameters.
...
[Butterworth](https://en.wikipedia.org/wiki/Butterworth_filter) lowpass has a maximally flat passband and monotonic frequency response.

`dcblock`

DC blocker (1st order)

frequency

1st order

Zero centers signal, countering any constant offset ("direct current").
...
Morphs between lowpass, peaking and highpass modes.
...
Turns white noise into pink noise.
...
Gain stays constant as Q is varied.

### [](https://crates.io/crates/fundsp#parameter-smoothing-filter)Parameter Smoothing Filter

The `follow` filter is special. It supports different rates for rising (attack) and falling (release) segments. For example, `afollow(0.1, 0.2)` has a 0.1 second attack and a 0.2 second release.
...
### [](https://crates.io/crates/fundsp#list-of-nonlinear-filters)List of Nonlinear Filters
...
[dirty biquad](https://jatinchowdhury18.medium.com/complex-nonlinearities-episode-4-nonlinear-biquad-filters-ae6b3f23cb0e)

Biquad with nonlinear state shaping and adjustable amplitude gain.
...
[feedback biquad](https://jatinchowdhury18.medium.com/complex-nonlinearities-episode-5-nonlinear-feedback-filters-115e65fc0402)

Biquad with nonlinear feedback and adjustable amplitude gain.
...
### [](https://crates.io/crates/fundsp#frequency-domain-resynthesis)Frequency Domain Resynthesis

Filtering and other effects can be done in the [frequency domain](https://en.wikipedia.org/wiki/Frequency_domain) as well with the `resynth` opcode.

The resynthesizer [Fourier transforms](https://en.wikipedia.org/wiki/Discrete-time_Fourier_transform) input windows to the frequency domain with an overlap of four. A processing function then translates these inputs to frequency domain outputs. The output windows are inverse transformed and overlap-added.
...
For example, to implement a highly selective [FFT](https://en.wikipedia.org/wiki/Fast_Fourier_transform) bandpass filter:
...
For more information on the technique, see [Fourier analysis and reconstruction of audio signals](http://msp.ucsd.edu/techniques/v0.11/book-html/node172.html).

## [](https://crates.io/crates/fundsp#more-on-multithreading-and-real-time-control)More On Multithreading And Real-Time Control
...
### [](https://crates.io/crates/fundsp#atomic-variables)Atomic Variables
...
### [](https://crates.io/crates/fundsp#settings)Settings

Settings are all kinds of node parameters with no dedicated inputs. A node can respond to one or more types of settings, which are applied using the `set` method.
...
To adjust settings from other threads, a node can be wrapped inside a setting listener with the `listen(node)` opcode. It returns a `(sender, node)` pair.

The sender can be cloned and invoked from other threads. The format of settings available through `sender` depends on the type of the graph.
...
If a listener is attached to a binary operation, then the different sides can be picked with the `left` or `right` method. For example, constants are exposed as scalar settings:
...
Later we can send filter parameters from anywhere via `sender`. For example, the lowpass filter supports setting cutoff frequency and Q value. Set filter cutoff to 2 kHz and Q to 2.0:
...
If a node responds to `center_q_gain`, then it also responds to `center_q` and `center`. If a node responds to `center_q`, then it also responds to `center`.

* * *

## [](https://crates.io/crates/fundsp#parametric-equalizer-recipe)Parametric Equalizer Recipe

In this example we make a 12-band, double precision parametric equalizer using the peaking `bell` filter. The sample type, as always, is 32-bit floating point (`f32`).
...
The type of the equalizer is `An<Chain<U12, FixedSvf<f64, BellMode<f64>>>>`. The equalizer is ready to use immediately. Filter samples:
...
Then, we can send settings to change center frequency, Q and gain. The settings are available because we constructed the filter using the `bell_hz` form. Set band 1 to amplify by 3 dB at 1000 Hz with Q set to 2.0:

    sender.try_send(Setting::center_q_gain(1000.0, 2.0, db_amp(3.0)).index(1)).expect("Cannot send setting.");
    

## [](https://crates.io/crates/fundsp#evcxr)evcxr

The `display` method returns information about a node, including an ASCII chart of the frequency response of channel 0.

Here is an example of using [`evcxr`](https://github.com/google/evcxr) to examine frequency responses interactively:
...
## [](https://crates.io/crates/fundsp#free-functions)Free Functions

These free functions are available in the environment.

* * *

### [](https://crates.io/crates/fundsp#component-opcodes)Component Opcodes
...
ADSR envelope. Attack time `a`, decay time `d`, sustain level `s`, and release time `r`. Input > 0.0 starts attack, input <= 0.0 starts release. Output in \[0.0, 1.0\].
...
Asymmetric smoothing filter with halfway attack time `a` seconds and halfway release time `r` seconds.
...
Nested allpass with inner allpass processing `x`.
...
Nested allpass with feedforward coefficient `c` and inner allpass processing `x`.
...
Allpass filter (2nd order) centered at `f` Hz with Q `q`.
...
Allpass filter (2nd order) with Q `q`.
...
Allpass filter (1st order). 2nd input is delay in samples (`delay` > 0).
...
Allpass filter (1st order) with `delay` at DC in samples (`delay` > 0).
...
Bandpass filter (2nd order) centered at `f` Hz with Q `q`.
...
Bandpass filter (2nd order) with Q `q`.
...
Resonant bandpass filter (2nd order).
...
Resonant bandpass filter (2nd order) centered at `f` Hz with resonance `q` in 0...1.
...
Resonant bandpass filter (2nd order) with resonance `q` in 0...1.

`bell()`

4 (audio, frequency, Q, gain)

1

Peaking filter (2nd order) with adjustable amplitude gain.
...
Peaking filter (2nd order) centered at `f` Hz with Q `q` and amplitude gain `gain`.
...
Peaking filter (2nd order) with Q `q` and amplitude gain `gain`.
...
Arbitrary [biquad filter](https://en.wikipedia.org/wiki/Digital_biquad_filter) with coefficients in normalized form.
...
[Brown](https://en.wikipedia.org/wiki/Brownian_noise) noise.
...
Butterworth lowpass filter (2nd order) with cutoff frequency `f` Hz.
...
Chorus effect with LFO seed `seed`, voice separation `sep` seconds, delay variation `var` seconds and LFO modulation frequency `mod` Hz.
...
Dirty biquad bell equalizer (2nd order) with feedback `shape`, for example, `Tanh(1.0)`.
...
Dirty biquad bell equalizer (2nd order) with feedback `shape`, center `f` Hz, Q value `q` and amplitude gain `gain`.
...
Zero center signal with cutoff frequency 10 Hz.
...
Zero center signal with cutoff frequency `f`.
...
Apply 10 ms of fade-in to signal.
...
Apply `t` seconds of fade-in to signal.
...
Delay of `t` seconds. Delay time is rounded to the nearest sample.
...
Dirty biquad highpass (2nd order) with feedback `shape`, for example, `Tanh(1.0)`.
...
Dirty biquad highpass (2nd order) with feedback `shape`, center `f` Hz and Q `q`.
...
Dirty biquad lowpass (2nd order) with feedback `shape`, for example, `Tanh(1.0)`.
...
Dirty biquad lowpass (2nd order) with feedback `shape`, center `f` Hz and Q `q`.
...
Dirty biquad resonator (2nd order) with feedback `shape`, for example, `Tanh(1.0)`.
...
Dirty biquad resonator (2nd order) with feedback `shape`, center `f` Hz and Q `q`.

`dsf_saw()`

2 (frequency, roughness)
...
Saw-like discrete summation formula oscillator with roughness `r` in 0...1.
...
Square-like discrete summation formula oscillator with roughness `r` in 0...1.
...
Feedback biquad bell equalizer (2nd order) with feedback `shape`, for example, `Tanh(1.0)`.
...
Feedback biquad bell equalizer (2nd order) with feedback `shape`, center `f` Hz, Q value `q` and amplitude gain `gain`.
...
Feedback Delay Network: enclose feedback circuit `x` (with equal number of inputs and outputs) using diffusive [Hadamard](https://en.wikipedia.org/wiki/Hadamard_matrix) feedback.
...
FIR filter with the specified weights, for example, `fir((0.5, 0.5))`.
...
Symmetric 3-point FIR calculated from desired `gain` at the Nyquist frequency.
...
Flanger effect with feedback amount `fb`, minimum delay `min_d` seconds, maximum delay `max_d` seconds and delay function `f`, e.g., `|t| lerp11(0.01, 0.02, sin_hz(0.1, t))`.
...
Feedback biquad highpass (2nd order) with feedback `shape`, for example, `Softsign(1.0)`.
...
Feedback biquad highpass (2nd order) with feedback `shape`, center `f` Hz and Q `q`.
...
Feedback biquad lowpass (2nd order) with feedback `shape`, for example, `Softsign(1.0)`.
...
Feedback biquad lowpass (2nd order) with feedback `shape`, center `f` Hz and Q `q`.
...
Smoothing filter with halfway response time `t` seconds.
...
Feedback biquad resonator (2nd order) with feedback `shape`, for example, `Softsign(1.0)`.
...
Feedback biquad resonator (2nd order) with feedback `shape`, center `f` Hz and Q `q`.
...
Bandlimited Hammond oscillator. Emphasizes first three partials.
...
Bandlimited Hammond oscillator at `f` Hz. Emphasizes first three partials.
...
Highpass filter (2nd order) with cutoff frequency `f` Hz and Q `q`.
...
Highpass filter (2nd order) with Q `q`.
...
Highpass filter (1st order) with cutoff frequency `f` Hz.
...
High shelf filter (2nd order) with adjustable amplitude gain.
...
High shelf filter (2nd order) centered at `f` Hz with Q `q` and amplitude gain `gain`.
...
High shelf filter (2nd order) with Q `q` and amplitude gain `gain`.
...
Sample-and-hold component with hold time variability `v` in 0...1.
...
Sample-and-hold component at `f` Hz with hold time variability `v` in 0...1.
...
Average together `U` channels. Inverse of `split`.
...
Look-ahead limiter with attack time (and latency) `a` seconds and release time `r` seconds.
...
Stereo look-ahead limiter with attack time (and latency) `a` seconds and release time `r` seconds.
...
[Lorenz dynamical system](https://en.wikipedia.org/wiki/Lorenz_system) oscillator.
...
Lowpass filter (2nd order) with cutoff frequency `f` Hz and Q `q`.
...
Lowpass filter (2nd order) with Q `q`.
...
1-pole lowpass filter (1st order).
...
1-pole lowpass filter (1st order) with cutoff frequency `f` Hz.
...
Resonant lowpass filter (2nd order).
...
Resonant lowpass filter (2nd order) centered at `f` Hz with resonance `q` in 0...1.
...
Resonant lowpass filter (2nd order) with resonance `q` in 0...1.
...
Low shelf filter (2nd order) with adjustable amplitude gain.
...
Low shelf filter (2nd order) centered at `f` Hz with Q `q` and amplitude gain `gain`.
...
Low shelf filter (2nd order) with Q `q` and amplitude gain `gain`.
...
Analyze input and output a summary according to the metering mode.
...
White [MLS noise](https://en.wikipedia.org/wiki/Maximum_length_sequence) source.
...
White MLS noise source from `n`\-bit MLS sequence (1 <= `n` <= 31).
...
Pass-through node that analyzes data passed through, storing a summary into the shared variable.
...
Moog resonant lowpass filter (4th order).
...
Moog resonant lowpass filter (4th order) with cutoff frequency `f` and resonance `q`.
...
Moog resonant lowpass filter (4th order) with resonance `q`.

`morph()`

4 (audio, frequency, Q, morph)

1

Morphing filter with morph input in -1...1 (-1 = lowpass, 0 = peaking, 1 = highpass)
...
Morphing filter with center frequency `f`, Q `q` and morph `morph` in -1...1 (-1 = lowpass, 0 = peaking, 1 = highpass)
...
Tapped delay line with cubic interpolation. Number of taps is `N`.
...
Tapped delay line with linear interpolation. Number of taps is `N`.
...
[White noise](https://en.wikipedia.org/wiki/White_noise) source. Synonymous with `white`.
...
Notch filter (2nd order) centered at `f` Hz with Q `q`.
...
Notch filter (2nd order) with Q `q`.
...
Bandlimited organ wave oscillator.
...
Bandlimited organ wave oscillator at `f` Hz.
...
Fixed mono-to-stereo equal power panner with pan in -1...1.
...
Mono-to-stereo equal power panner with pan in -1...1.
...
Peaking filter (2nd order) centered at `f` Hz with Q `q`.
...
Peaking filter (2nd order) with Q `q`.
...
Phaser effect with feedback amount `fb` and modulation function `f`, e.g., `|t| sin_hz(0.1, t) * 0.5 + 0.5`.
...
[Pink noise](https://en.wikipedia.org/wiki/Pink_noise) source.
...
[Karplus-Strong](https://en.wikipedia.org/wiki/Karplus%E2%80%93Strong_string_synthesis) plucked string oscillator with frequency `f` Hz, `gain` per second (`gain` <= 1) and high frequency `damping` in 0...1.
...
Bandlimited pulse wave with duty cycle in 0...1.
...
Non-bandlimited ramp (sawtooth) wave in 0...1.
...
Non-bandlimited ramp (sawtooth) wave in 0...1 with frequency `f` Hz.
...
Non-bandlimited ramp (sawtooth) wave in 0...1 with initial `phase` in 0...1.
...
Non-bandlimited ramp (sawtooth) wave in 0...1 with frequency `f` Hz and initial `phase` in 0...1.
...
Constant-gain bandpass resonator (2nd order).
...
Constant-gain bandpass resonator (2nd order) with center frequency `f` Hz and Q `q`.
...
Stereo reverb (32-channel [FDN](https://ccrma.stanford.edu/~jos/pasp/Feedback_Delay_Networks_FDN.html)) with room size `r` meters (10 is average), reverberation time `t` seconds and high frequency damping `d` (in 0...1).
...
Another stereo reverb (32-channel hybrid [FDN](https://ccrma.stanford.edu/~jos/pasp/Feedback_Delay_Networks_FDN.html)) with room size `r` meters (10-30 meters is supported), reverberation time `t` seconds, diffusion amount `d` (in 0...1), modulation speed `m` (nominal range 0...1, beyond starts being an effect), and loop filter `f`.
...
Another stereo reverb (allpass loop) with reverberation time `t` seconds, diffusion amount `d` (in 0...1), and loop filter `f`.
...
[Rössler dynamical system](https://en.wikipedia.org/wiki/R%C3%B6ssler_attractor) oscillator.
...
Rotate stereo signal `a` radians with gain `g`.
...
Bandlimited saw wave oscillator.
...
Bandlimited saw wave oscillator at `f` Hz.
...
Shape signal with waveshaper mode `mode`.
...
Shape signal with waveshaper function `f`, e.g., `tanh`.
...
Sine oscillator with initial phase `p` in 0...1.
...
Bandlimited soft saw wave oscillator.
...
Bandlimited soft saw wave oscillator at `f` Hz.
...
Bandlimited square wave oscillator at frequency `f` Hz.
...
Tapped delay line with cubic interpolation. All times are in seconds.
...
Tapped delay line with linear interpolation. All times are in seconds.
...
Bandlimited triangle wave oscillator at `f` Hz.
...
Output value of the shared variable.
...
Play back a channel of `Arc<Wave>`. Optional loop point is the index to jump to at the end of the wave.
...
Play back a channel of `Arc<Wave>` between indices `start` (inclusive) and `end` (exclusive), with optional `loop` index to jump to at the end.
...
[White noise](https://en.wikipedia.org/wiki/White_noise) source. Synonymous with `noise`.
...
#### [](https://crates.io/crates/fundsp#subsampled-controls)Subsampled Controls

`envelope` is a node that samples a time varying control function. For example, `envelope(|t| exp(-t))` is an exponentially decaying [envelope](https://en.wikipedia.org/wiki/Envelope_(music)). A control function is something that is expected to change relatively slowly. Therefore, we can save time by not calling it at every sample.
...
#### [](https://crates.io/crates/fundsp#indexed-and-fractional-generator-functions)Indexed And Fractional Generator Functions

`branchi`, `busi`, `pipei`, `sumi` and `stacki` are opcodes that combine multiple nodes of the same type. The number of nodes is their first generic argument. They accept a generator function that is issued `u64` integers starting from 0. The nodes are allocated inline, and each node is assigned its own pseudorandom phase.
...
#### [](https://crates.io/crates/fundsp#waveshaping-modes)Waveshaping Modes

These are arguments to the `shape` opcode. Shapes `Atan`, `Clip`, `Softsign` and `Tanh` all have a slope of 1 at the origin when hardness is 1, saturate in the range -1...1, and are nonexpansive up to a hardness of 1.

-   `Adaptive::new(timescale, inner)`: Apply adaptive normalizing distortion with smoothing `timescale` in seconds. Smoothing timescale is the time it takes for level estimation to move halfway to a new value. The argument to the `inner` shape is divided by the RMS level of the signal.
...
#### [](https://crates.io/crates/fundsp#metering-modes)Metering Modes
...
### [](https://crates.io/crates/fundsp#math-and-utility-functions)Math And Utility Functions
...
[A-weighted](https://en.wikipedia.org/wiki/A-weighting) amplitude response at `f` Hz (normalized to 1.0 at 1 kHz)
...
convert [MIDI](https://en.wikipedia.org/wiki/MIDI) note number `x` to Hz (69.0 = _A4_ = 440 Hz)
...
[M-weighted](https://en.wikipedia.org/wiki/ITU-R_468_noise_weighting) amplitude response at `f` Hz (normalized to 1.0 at 1 kHz)
...
[Catmull-Rom](https://en.wikipedia.org/wiki/Cubic_Hermite_spline#Catmull%E2%80%93Rom_spline) cubic interpolation between `x1` and `x2`, taking `x0` and `x3` into account

`spline_mono(x0, x1, x2, x3, t)`

[monotonic cubic interpolation](https://en.wikipedia.org/wiki/Monotone_cubic_interpolation) between `x1` and `x2`, taking `x0` and `x3` into account
...
### [](https://crates.io/crates/fundsp#easing-functions)Easing Functions

_Easing functions_ are interpolation curves that remap the range 0...1. These math functions have the shape of an easing function.
...
![](https://github.com/SamiPerttu/fundsp/raw/HEAD/easing_functions.png "easing_functions")
...
### [](https://crates.io/crates/fundsp#noise-functions)Noise Functions

![](https://github.com/SamiPerttu/fundsp/raw/HEAD/fractal_noise.png "fractal_noise example")

![](https://github.com/SamiPerttu/fundsp/raw/HEAD/ease_noise.png "ease_noise example")

![](https://github.com/SamiPerttu/fundsp/raw/HEAD/fractal_ease_noise.png "fractal_ease_noise example")

* * *

## [](https://crates.io/crates/fundsp#examples)Examples
...
stereo-to-mono mixdown (inverse of mono-to-stereo splitter)
...
replace left channel with silence
...
replace right channel with silence
...
2nd order and 1-pole lowpass filters in series (3rd order)
...
double resonator in series (4th order)
...
Oversampled [FM (frequency modulation)](https://ccrma.stanford.edu/~jos/sasp/Frequency_Modulation_FM_Synthesis.html) oscillator at `f` Hz with modulation index `m`
...
frequency doubled dual sine oscillator
...
1 second feedback delay with 3 dB attenuation
...
1 second ping-pong delay with 6 dB attenuation.
...
1 second delay with wet/dry mix controlled by shared variable `wet`. The shared variable can be declared as `let wet = shared(0.5);`
...
A null node. Stacking it with another node modifies its sound subtly, as the hash is altered.
...
Hot-rodded null node outfitted with a custom hash. Uses more electricity.

* * *

### [](https://crates.io/crates/fundsp#examples-from-the-prelude)Examples From The Prelude
...
`white() >> lowpole_hz(10.0) * constant(13.7)`
...
### [](https://crates.io/crates/fundsp#equivalent-expressions)Equivalent Expressions
...
### [](https://crates.io/crates/fundsp#audio-graph-types)Audio Graph Types
...
The preludes employ the wrapper type `An<X: AudioNode>` containing operator overloads and other trait implementations. The wrapper also implements the `AudioUnit` trait.
...
It is also possible to declare the return type as an `impl AudioNode`. However, this type is also opaque and cannot be stored as an `AudioNode`; conversion to `AudioUnit` may be necessary in this case.
