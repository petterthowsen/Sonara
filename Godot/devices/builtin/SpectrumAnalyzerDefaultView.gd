...
static var logger := Log.make("SpectrumVisualizer")
...
func _on_bind() -> void:
	logger.debug("_on_bind channel=", channel_id, " pos=", device_position, " device=", device != null)
	...
	logger.debug("smoothing param id=", _smoothing_param_id)
	# Resolve resolution (FFT Size) parameter
	_resolution_param_id = device.get_parameter_id_by_name("FFT Size")
	logger.debug("fft size param id=", _resolution_param_id)
	...
	if _smoothing_param_id >= 0:
		...
		if p:
			logger.debug("Speed enum_values=", p.enum_values)
		else:
			logger.debug("Speed param metadata missing")
	if _resolution_param_id >= 0:
		var pr = device.get_parameter(_resolution_param_id)
		if pr:
			logger.debug("FFT Size enum_values=", pr.enum_values)
		else:
			logger.debug("FFT Size param metadata missing")


func _on_view_shown() -> void:
	logger.debug("_on_view_shown subscribe; renderer_ready=", renderer != null, " ch=", (device.channel_id if device else -1), " pos=", (device.position if device else -1))
	...
	if _smoothing_param_id >= 0:
		...
		logger.debug("applying initial speed normalized=", n)
		_apply_speed_from_param(n)
	
	if _resolution_param_id >= 0:
		var rn = device.get_parameter_normalized(_resolution_param_id)
		logger.debug("applying initial resolution normalized=", rn)
...
func _on_view_hidden() -> void:
	logger.debug("_on_view_hidden unsubscribe")
	# Unsubscribe
	if device:
...
func _on_device_parameter_changed(param_id: int, value: float) -> void:
	...
	logger.debug("param_changed id=", param_id, " value=", value)
	if param_id == _smoothing_param_id:
		logger.debug("speed change received normalized=", value)
		_apply_speed_from_param(value)
	
	elif param_id == _resolution_param_id:
		logger.debug("resolution change received normalized=", value)
...
func _apply_speed_from_param(normalized: float) -> void:
	...
	logger.debug("_apply_speed_from_param normalized=", normalized, " enum_len=", enum_len, " idx=", idx)
	match idx:
		0:
			renderer.smoothing = 1.0
			logger.debug("speed=Freeze smoothing=", renderer.smoothing)
		1:
			renderer.smoothing = 0.85
			logger.debug("speed=Slow smoothing=", renderer.smoothing)
		2:
			renderer.smoothing = 0.5
			logger.debug("speed=Medium smoothing=", renderer.smoothing)
		_:
			renderer.smoothing = 0.1
			logger.debug("speed=Fast smoothing=", renderer.smoothing)


func _apply_resolution_enum(normalized: float) -> void:
	var idx: int = int(round(clamp(normalized, 0.0, 1.0) * 3.0))
	logger.debug("_apply_resolution_enum normalized=", normalized, " idx=", idx)
	match idx:
		0:
			logger.debug("using tiny")
			renderer.resolution = SpectrumRenderer.Resolution.Tiny  # 512
		1:
			logger.debug("using small")
			renderer.resolution = SpectrumRenderer.Resolution.Small # 1024
		2:
			logger.debug("using medium")
			renderer.resolution = SpectrumRenderer.Resolution.Medium # 2048
		_:
			logger.debug("using large")
...
func _apply_scale_from_param(normalized: float) -> void:
	...
	logger.debug("scale set=", ("Log" if idx == 0 else "Linear"))
...
func _apply_style_from_param(normalized: float) -> void:
	...
	logger.debug("style set=", ("BARS" if idx == 0 else "LINE"))
	...
	if device and osc_path == device.osc_path():
		if renderer:
			...
			if (_spectrum_count % 30) == 1:
				logger.debug("spectrum update #", _spectrum_count, " len=", spectrum.size())
		else:
